...
from prompts import SYSTEM_PROMPT, SYSTEM_PROMPT_FOR_SUMMARIZE
...
class ChatBot:
    ...
    def __init__(self):
        ...
        self.MAX_MESSAGES_FOR_SPLIT = 10
        self.MAX_MESSAGES_FOR_SUMMARIZE = 10
    ...
    def add_message(self, role: str, content: str):
        ...
        #я так понимаю, нужно будет выбрать что то одноЮ либо отрезать либо суммаризировать. Суммаризация лучший вариант. Для корректной работы нужно закомментировать код ниже
        if len(self.conversation_history) > self.MAX_MESSAGES_FOR_SPLIT:
            prompt = self.conversation_history[0]
            last_messages = self.conversation_history[-self.MAX_MESSAGES_FOR_SPLIT:]
    ...
    def send_message(self, user_message: str) -> Optional[str]:
        ...
        try:
            ...
            with console.status("[bold green]🤔 Думаю...", spinner="dots"):
                # Проверяем нужно делать суммаризацию или нет
                if len(self.conversation_history) > self.MAX_MESSAGES_FOR_SUMMARIZE:
