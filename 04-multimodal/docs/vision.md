...
1. **bot.py** - точка входа
   ...
   - Загружает модель Whisper (модель "base") для транскрипции голосовых сообщений
   - Сохраняет модель Whisper в dispatcher для использования в handlers
...
2. **handlers.py** - обработка событий
   ...
   - Обработчик голосовых сообщений → скачивание файла → транскрипция через Whisper → извлечение транзакций через LLM → сохранение транзакций → показ ответа + статус + баланс
...
3. **llm.py** - интеграция с LLM и Whisper
   ...
   - Метод `get_voice_transcribe()` - транскрипция голосовых сообщений через Whisper (язык: русский)
...
```
Telegram → handlers.py (скачать голосовой файл) → llm.py (Whisper транскрипция) → 
handlers.py (транскрибированный текст) → llm.py (structured output) → OpenRouter/Ollama → 
llm.py → handlers.py (извлечь транзакции, сохранить в transactions, показать ответ + статус + баланс) → Telegram
```
...
**3. Транскрипция голосовых сообщений (Whisper):**
```python
async def get_voice_transcribe(file_path: str, model) -> str:
    # Транскрипция через Whisper (язык: русский)
...
- **Whisper для голосовых сообщений**: Модель Whisper загружается при старте бота (модель "base"), используется для транскрипции голосовых сообщений на русском языке
- **Обработка голосовых сообщений**: Голосовое сообщение транскрибируется через Whisper, затем транскрибированный текст обрабатывается так же, как текстовое сообщение
...
3. Бот отправляет файл в Whisper для транскрипции (язык: русский)
4. Бот получает транскрибированный текст
5. Бот отправляет транскрибированный текст в LLM со structured output
6. LLM извлекает транзакции из текста
7. Бот сохраняет транзакции в `transactions[chat_id]`
8. Бот рассчитывает баланс
9. Бот отправляет пользователю транскрибированный текст + ответ LLM + статус + баланс
10. Бот удаляет временный голосовой файл
