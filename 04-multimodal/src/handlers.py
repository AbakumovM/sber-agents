...
from llm import get_transaction_response_text, get_transaction_response_image, get_voice_transcribe
...
async def handler_voice(message: Message, whisper_model: Any, bot: Bot):
    logger.info(f'Пользователь отправил голосовое сообщение, начинаю обработку ...')
    ...
    logger.info(f'Файл скачен')
    await message.answer("⏳ Обрабатываю голосовое сообщение...")

    result = await get_voice_transcribe(voice_file, whisper_model)
    ...
    try:
        ...
        response = await get_transaction_response_text(result, message_history)
        ...
        if response.transactions:
            ...
            logger.warning(f"No transactions extracted from message: '{result}' for {chat_id}")
